...
(function () {
	...
	captureButton.onclick = function (event) {
		...
		let checkboxSavePage = document.getElementById('checkboxSavePage');
		...
		if (checkboxSavePage.checked) {
			chrome.extension.sendMessage({ action: "getDevToolsStatus" }, function (response) {
				if (!response.data) {
					alert("DevTools needs to be open to get HAR logs for saving page")
				} else {
					let message = { action: "savePage" };
					chrome.extension.sendMessage(message, function (a) {
						// alert(JSON.stringify(a));
					});
